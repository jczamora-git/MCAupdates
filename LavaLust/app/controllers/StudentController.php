...
class StudentController extends Controller
{
    ...
    private function require_admin_or_adviser()
    {
        ...
        if (!in_array($role, ['admin', 'teacher'], true)) {
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Access denied. Admin or adviser only.'
            ]);
            return false;
        }
    ...
    /**
     * Check if a student has a linked portal account (student or enrollee role)
     * GET /api/students/{id}/portal-account
     */
    public function api_get_portal_account($id = null)
    {
        api_set_json_headers();

        if (!$this->require_admin_or_adviser()) {
            return;
        }
        ...
        if (!empty($student['user_id'])) {
            $user = $this->db->table('users')
                ->select('id, role, email, phone, status')
                ->where('id', (int)$student['user_id'])
                ->get();

            if ($user && in_array($user['role'] ?? '', ['student', 'enrollee'], true)) {
                $hasAccount = true;
            } else {
                $user = null;
            }
